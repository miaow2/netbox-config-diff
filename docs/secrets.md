...
You can change the precedence order via plugin configuration `SECRETS_PRECEDENCE`. It accepts a list of attribute names (or a comma-separated string) that will be resolved against the `Device` object. Use the special name `device` to refer to the device itself. Invalid names are ignored. Default value:
